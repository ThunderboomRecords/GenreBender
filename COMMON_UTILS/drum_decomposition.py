...
def drumGridFromHs(Hs: list, grid_size: int = 48) -> np.ndarray:
    """
    Parameters
    ----------
    Hs : list[np.ndarray]
...
def quantiseGrids(grids: list) -> np.ndarray:
...
def makeGridPrior(grid_size: int = 48, alpha: float = 0.5) -> np.ndarray:
...
def getPriorMap(p: np.ndarray) -> np.ndarray:
...
def separateSamples(y: np.ndarray, sr: int, onsets: np.ndarray, hop_length: int = 512) -> list:
    if len(onsets) <= 1:
        return []
...
def longestRun(arr: np.ndarray) -> tuple:
...
def trimSample(y: np.ndarray, padding: int = 5, hop_length: int = 512) -> np.ndarray:
...
def isolatePeak(H: np.ndarray, index: int):
